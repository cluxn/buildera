...
export async function GET(request: NextRequest) {
  ...
  const [posts, cs, guides] = await Promise.all([
    query<{ title: string; slug: string; excerpt: string }>(
      `SELECT title, slug, excerpt FROM blog_posts WHERE status='PUBLISHED' AND (title LIKE ? OR excerpt LIKE ?) LIMIT 5`,
      [like, like],
    ),
    query<{ title: string; slug: string }>(
      `SELECT title, slug FROM case_studies WHERE status='PUBLISHED' AND title LIKE ? LIMIT 5`,
      [like],
    ),
    query<{ title: string; slug: string; excerpt: string }>(
