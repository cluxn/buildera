import Image from 'next/image'
import type { Testimonial } from '@/lib/api'

export function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  return (
    <div className="break-inside-avoid bg-background border border-border rounded-xl p-6 mb-4">
      <p className="text-yellow-500 text-lg mb-3">{'★'.repeat(testimonial.rating)}</p>
      <p className="text-base italic mb-4 leading-relaxed">&ldquo;{testimonial.quote}&rdquo;</p>
      {testimonial.logo_url && (
        <div className="mb-3">
          <Image src={testimonial.logo_url} alt={testimonial.company ?? ''} width={40} height={40} sizes="40px" className="object-contain" />
        </div>
      )}
      <p className="font-semibold">{testimonial.person_name}</p>
      {(testimonial.person_title || testimonial.company) && (
        <p className="text-sm text-muted-foreground">
          {[testimonial.person_title, testimonial.company].filter(Boolean).join(', ')}
        </p>
      )}
      {testimonial.service_category && (
        <span className="inline-block mt-2 text-xs px-2 py-0.5 rounded-full bg-[var(--brand-primary)]/10 text-[var(--brand-primary)]">
          {testimonial.service_category}
        </span>
      )}
    </div>
  )
}
